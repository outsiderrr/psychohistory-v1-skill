...
>
> **⚠️ 环境约束：** Phase 1 的女娲路径**仅适用于 Claude Code CLI 等已安装 `huashu-nuwa` 的环境**。`huashu-nuwa` 不是任何 chat AI 的内置 skill，它是独立开源项目（github.com/alchaincyf/nuwa-skill），需要通过 `npx skills add alchaincyf/nuwa-skill` 安装，安装后**只能在 CLI 环境运行**。在 Claude.ai / ChatGPT / Gemini 等 chat AI 环境中，Phase 1 自动降级为 Appendix Research Hand-off Template；character-toolkit 的 Export Mode 会强制走 fallback 路径并在输出中注明。
...
**环境检查**：如果执行环境没有 `huashu-nuwa` Skill（例如 chat AI 环境，或 CLI 未安装），**停止 primary path，跳转到本文件末尾的 Appendix Research Hand-off Template 执行 fallback**，并在 references.md §1 Source Materials 顶部注明 "Generated via Research Hand-off fallback; Nuwa skill unavailable in this environment."
