...
>
> **⚠️ 环境约束：** Phase 1 的女娲路径**仅适用于已安装 `huashu-nuwa` 的 CLI 环境**（Claude Code 等）。`huashu-nuwa` 不是任何 chat AI 的内置 skill，它是独立开源项目（github.com/alchaincyf/nuwa-skill），需要通过 `npx skills add alchaincyf/nuwa-skill` 安装。如果当前 CLI 未安装 `huashu-nuwa`，跳转到本文件末尾的 Appendix Research Hand-off Template 作为正常 fallback 执行。
...
- **自动模式**（默认）：连续执行 Phase 0-4，中间不停
- **交互模式**：每个 Phase 完成后让用户审阅再继续（需用户显式请求，如"一步一步"/"interactive"）
...
调用 `huashu-nuwa` Skill（需通过 `npx skills add alchaincyf/nuwa-skill` 安装到 Claude Code CLI；**不是任何 chat AI 的内置 skill**），对上述目标人物进行深度调研和思维框架提炼。

**环境检查**：如果当前 CLI 未安装 `huashu-nuwa` Skill，**停止 primary path，跳转到本文件末尾的 Appendix Research Hand-off Template 执行**，并在 references.md §1 Source Materials 顶部注明 "Generated via Research Hand-off path; huashu-nuwa skill not installed in this CLI."
...
This template is the CLI path used when `huashu-nuwa` is not installed. `huashu-nuwa` is the preferred primary path because its 6-agent parallel distillation produces stronger cognitive extraction, counter-evidence surfacing, and expression DNA than a single-prompt research call. When `huashu-nuwa` is not installed, this Research Hand-off path takes over — it is a fully supported CLI path (orchestrated by `character-toolkit/SKILL.md` via the Research Hand-off skill), not a degraded workaround.
